(ns run-tests
  ...
  The lake Worker range-reads Parquet footers and column chunks as ClojureScript. `lake_e2e_test.clj` stays JVM-only, so counts differ by design -- this is an ADDITION to the JVM gate, not a replacement.
